...
try:
    ...
    class LivePDF(FPDF):
        ...
        def add_title_page(self, session):
            self.add_page()
            self._bg()
        ...
        def add_summary_page(self, session):
            self.add_page()
            self._bg()
...
def _step_generate_contract(session: LiveSession, config, pdf, prompt: str) -> tuple[str, float]:
    """Step 1: Generate contract via LLM. Returns (readme, duration_ms)."""
...
def _step_parse_blocks(session: LiveSession, readme: str) -> tuple[list, float, bool]:
    ...
    print()
    return blocks, dur_parse, bool(blocks)


def _step_validate_blocks(session: LiveSession, blocks: list) -> bool:
...
def _step_analyze_blocks(session: LiveSession, blocks: list) -> list:
    """Step 4: Analyze block content. Returns analysis_checks."""
...
def _step_sha_integrity(session: LiveSession, readme: str, blocks: list) -> list:
    """Step 5: SHA-256 integrity check. Returns sha_checks."""
...
def _save_outputs(session: LiveSession, pdf, readme: str, prompt_key: str) -> None:
...
def _print_summary(session: LiveSession, blocks: list, readme_path: Path):
    """Print final summary."""
    hdr("PODSUMOWANIE")
    ...
    print(f"  {BOLD}Prompt:{NC}      {session.prompt_key}")
    print(f"  {BOLD}Model:{NC}       {session.model}")
